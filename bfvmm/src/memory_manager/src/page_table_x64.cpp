...
bool page_table_x64::add_entry(void *physical_address, void *virtual_address, uint64_t flags)
{
    bool rc = true;
...
uint64_t *page_table_x64::pdp(void *virtual_address, bool alloc)
{
    ...
    // When we do the lookup in our phys2virt map, we have to remove
    // the flags that we've been setting to get the correct entries
    entry &= ~PAGE_FLAGS;
...
uint64_t *page_table_x64::pt(void *virtual_address, bool alloc)
{
    ...
    if((pgd_entry & PAGE_PRESET_FLAG) == 0 && alloc)
    {
        ...
        pgd(virtual_address)[PGD_OFFSET(virtual_address)] |= (PAGE_RW_FLAG|PAGE_PRESET_FLAG);
    ...
    // When we do the lookup in our phys2virt map, we have to remove
    // the flags that we've been setting to get the correct entries
    pgd_entry &= ~PAGE_FLAGS;

    return (uint64_t*)g_mm->phys_to_virt((void*)pgd_entry);
}

bool page_table_x64::add_table_entry_generic(uint64_t *table, void *phys_addr, uint16_t offset, uint64_t flags)
{
    table[offset] = (uint64_t)phys_addr;
    table[offset] |= (PAGE_PRESET_FLAG|flags);

    return true;
}

bool page_table_x64::add_pdp_entry(void *physical_address, void *virtual_address, uint64_t flags)
{
    uint16_t pdp_offset = PDP_OFFSET(virtual_address);

    return add_table_entry_generic(pdp(virtual_address), physical_address, pdp_offset, flags);
}

bool page_table_x64::add_pgd_entry(void *physical_address, void *virtual_address, uint64_t flags)
{
    uint16_t pgd_offset = PGD_OFFSET(virtual_address);

    return add_table_entry_generic(pgd(virtual_address), physical_address, pgd_offset, flags);
}

bool page_table_x64::add_pt_entry(void *physical_address, void *virtual_address, uint64_t flags)
{
    uint16_t pt_offset = PT_OFFSET(virtual_address);

    return add_table_entry_generic(pt(virtual_address), physical_address, pt_offset, flags);
}
