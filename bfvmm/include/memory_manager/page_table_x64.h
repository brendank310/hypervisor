...
#define PAGE_PRESET_FLAG            (1ull<<0)
#define PAGE_RW_FLAG                (1ull<<1)
#define PAGE_USERSPACE_FLAG         (1ull<<2)
#define PAGE_WRITE_THROUGH_FLAG     (1ull<<3)
#define PAGE_CACHE_DISABLED_FLAG    (1ull<<4)
#define PAGE_ACCESSED_FLAG          (1ull<<5)
#define PAGE_RESERVED_FLAG          (1ull<<6)
#define PAGE_SIZE_FLAG              (1ull<<7)
#define PAGE_EXEC_FLAG              (1ull<<63)
...
public:
    ...
    ///
    virtual bool add_entry(void *physical_address, void *virtual_address, uint64_t prot);
...
private:
    ...
    bool add_table_entry_generic(uint64_t *table, void *phys_addr, uint16_t offset, uint64_t flags = PAGE_RW_FLAG|PAGE_EXEC_FLAG);

    bool add_pml4_entry(void *physical_address, void *virtual_address, uint64_t flags = PAGE_RW_FLAG|PAGE_EXEC_FLAG);

    bool add_pdp_entry(void *physical_address, void *virtual_address, uint64_t flags = PAGE_RW_FLAG|PAGE_EXEC_FLAG);

    bool add_pgd_entry(void *physical_address, void *virtual_address, uint64_t flags = PAGE_RW_FLAG|PAGE_EXEC_FLAG);

    bool add_pt_entry(void *physical_address, void *virtual_address, uint64_t flags = PAGE_RW_FLAG|PAGE_EXEC_FLAG);
